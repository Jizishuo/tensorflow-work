...
data1 = pd.read_csv('F:/work/tianhe4location/basic_data/orig_data/tianhe4location521-527.csv')
...
full_cellname = pd.read_csv('F:/work/tianhe4location/basic_data/cellname_4loc.csv')
...
for i in range(len(result.index)) :
    cell_list.append(result.index[i][0])
net_list = list(set(cell_list))  
stand_times = 2  
...
def standard_data(result, net_list, stand_times):
    ...
    for net_num in net_list:
        for hour_num in hours:
            ...
            data_list = DataFrame(data_list).T
            data_list.columns = ['net_num', 'hour', 'UE', 'erab', 'flow', 'handover', 'rrc']
            
            data_standard.append(data_list)
    out_standard = DataFrame(data_standard)
    out_standard.columns = ['net_num', 'hour', 'UE', 'erab', 'flow', 'handover', 'rrc']
...
result.to_csv('F:/work/tianhe4location/result2127.csv')
